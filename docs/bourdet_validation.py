...
def main() -> None:
    ...
    ax.loglog(dt, paper_L01, "rs", markersize=3, alpha=0.6, label=r"Derivative $L=0.1$")
    ax.loglog(dt, paper_L0, "b^", markersize=3, alpha=0.6, label=r"Derivative $L=0$")
    ...
    ax.loglog(dt, dp, "ko", markersize=4, label=r"$\Delta p$")
    ax.loglog(dt, np.abs(our_L01), "rs", markersize=3, alpha=0.6, label=r"Derivative $L=0.1$")
    ax.loglog(dt, np.abs(our_L0), "b^", markersize=3, alpha=0.6, label=r"Derivative $L=0$")
    ax.set_xlabel(r"$\Delta t$ (hours)", fontsize=10)
    ax.legend(fontsize=8, loc="lower right")
    ax.grid(True, which="both", alpha=0.3)
    ax.set_ylim(0.1, 1000)
    ...
    print(
        f"  L=0:   max rel err = {rel_err_L0[interior].max():.4e}%,"
        f" mean = {rel_err_L0[interior].mean():.4e}%"
    )
    print(
        f"  L=0.1: max rel err = {rel_err_L01[interior].max():.4e}%,"
