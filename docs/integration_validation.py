...
def main() -> None:
    ...
    # ----------------------------------------------------------------
    ple_cases = [
        ('PLE (Di=0.005, Dinf=1e-5, n=0.5)',
         dca.PLE(qi=10_000.0, Di=0.005, Dinf=1e-5, n=0.5)),
        ('PLE (Di=0.1, Dinf=1e-4, n=0.5)',
         dca.PLE(qi=10_000.0, Di=0.1, Dinf=1e-4, n=0.5)),
        ('PLE (Di=1.0, Dinf=0.01, n=0.5)',
         dca.PLE(qi=10_000.0, Di=1.0, Dinf=0.01, n=0.5)),
        ('PLE (Di=0.5, Dinf=5e-4, n=0.3)',
         dca.PLE(qi=10_000.0, Di=0.5, Dinf=5e-4, n=0.3)),
    ]

    ple_models = {}
    for label, model in ple_cases:
        fn = model._qfn
        ple_models[label] = (lambda m=model: m.cum(t),
                             lambda f=fn: reference_cum(f, t))

    make_figure(ple_models, t,
    ...
    for label, primary in sec_cases:
        ...
        fn = sec._qfn
        sec_models[label] = (lambda s=sec: s.cum(t),
                             lambda f=fn: reference_cum(f, t))

    make_figure(sec_models, t,
